...
private:
    ...
    size_t numItem;   // size_t because used to return size()
    double resizeVal; // resize alpha value
};
...
{
    //like vector class for desctructor
...
// To be completed -- done? check if deleted
/**
 * @brief Returns true if the table has no non-deleted key,value pairs,
 *        and false otherwise
 *
...
{
    // check the loading factor >0.5 (resizeAlpha) = rize()
    // double tableSize = static_cast<double> table_.size();
    if ((((double)numItem) / ((double)table_.size())) >= resizeVal)
    {
        resize();
    }

    // create value
    HASH_INDEX_T hash = this->probe(p.first);
...
{
    mIndex_++;
    if (mIndex_ >= table_.size()) // chose a random number
    {
    ...
    std::vector<HashItem *> resizedTable(CAPACITIES[mIndex_]);
    
    // new table to reinsert
    table_ = resizedTable;
    ...
    for (int i = 0; i < prevTable.size(); i++)
    {
        if (prevTable[i] != nullptr)
        {
            ...
            {
                // insert element starting from left to right of old table
